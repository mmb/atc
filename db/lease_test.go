	...
	Describe("EnsurePendingBuildExists", func() {
		Context("when only a started build exists", func() {
			BeforeEach(func() {
				...
				started, err := sqlDB.StartBuild(build1.ID, pipelineDB.GetPipelineID(), "some-engine", "some-metadata")
				Expect(err).NotTo(HaveOccurred())
			...
			It("doesn't create another build the second time it's called", func() {
				...
				started, err := sqlDB.StartBuild(build2.ID, pipelineDB.GetPipelineID(), "some-engine", "some-metadata")
				Expect(err).NotTo(HaveOccurred())
				Expect(started).To(BeTrue())

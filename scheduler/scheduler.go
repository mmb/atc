...
type SchedulerDB interface {
	...
	LeaseResourceCheckingForJob(logger lager.Logger, job string, interval time.Duration) (db.Lease, error)
	IsPaused() (bool, error)
...
func (s *Scheduler) TriggerImmediately(
	...
	lease, err := s.DB.LeaseResourceCheckingForJob(logger, jobConfig.Name, 5*time.Minute)
	if err != nil {
